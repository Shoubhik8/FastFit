...
def multi_ref_cloth_agnostic_mask(
    ...
    **kwargs,
    ...
    # Strong Protect Area (Hands, Face, Accessory, Feet)
    # Preserve hands/feet (where DensePose says hand/foot AND SCHP says arm/leg) so they are
    # NOT regenerated by the whole-outfit inpaint. Mirrors cloth_agnostic_mask() above; without
    # this the multiref mask sweeps the feet in and the model hallucinates footwear.
    ...
    # Do NOT force the feet into the mask — they are preserved via strong_protect_area below.
    mask_dense_area = part_mask_of(
        MASK_DENSE_PARTS["overall"], densepose_mask, DENSE_INDEX_MAP
    )
    mask_dense_area = cv2.resize(
        mask_dense_area.astype(np.uint8),
        None,
    ...
    # 凸包扩张
    mask_area = hull_mask(mask_area * 255) // 255  # Convex Hull to expand the mask area
    # Re-exclude the strong protect area after the hull (mirrors cloth_agnostic_mask) so the
    # convex hull can't swallow the hands/feet it just expanded over.
